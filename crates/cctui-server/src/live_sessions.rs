//! `idx_sessions_live` (migration 121) is partial on exactly
...
//! requires rewording migration 121.
...
mod tests {
    ...
    fn predicate_matches_the_partial_index_in_migration_117() {
        let migration = include_str!("../../../migrations/121_sessions_live.up.sql");
        assert!(
            migration.contains(&format!("WHERE {}", live_sessions_predicate!())),
            "migration 121's index predicate drifted from live_sessions_predicate!"
        );
    }
